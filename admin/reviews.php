            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Course / Module</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Rating</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Review</th>
...
<?php foreach ($reviews as $rv): ?>
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <p class="text-sm font-medium text-gray-800"><?php echo htmlspecialchars($rv['module_name']); ?></p>
                            <p class="text-xs text-gray-400"><?php echo htmlspecialchars($rv['course_name']); ?></p>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-0.5">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <svg class="w-4 h-4 <?php echo $i <= $rv['rating'] ? 'text-yellow-400' : 'text-gray-300'; ?>" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                <?php endfor; ?>
                                <span class="ml-1.5 text-xs font-bold text-gray-500"><?php echo $rv['rating']; ?>/5</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600 max-w-xs">
                            <?php echo htmlspecialchars($rv['review'] ?: '<span class="text-gray-400 italic">No written review</span>'); ?>
                        </td>
