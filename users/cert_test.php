...
$existing = $conn->query("SELECT id FROM certificates WHERE certificate_no = '$certId'");
if ($existing && $existing->num_rows > 0) {
    if (isset($_GET['validate'])) {
        header('Content-Type: application/json');
        echo json_encode(['ok' => false, 'error' => "You have already downloaded the certificate for this course."]);
        exit;
    }
    http_response_code(409);
    die("You have already downloaded the certificate for this course.");
}
...
echo json_encode(['pdf' => $pdfContent, 'filename' => $filename]);
exit;